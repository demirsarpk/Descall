...
- **Download page** is Windows-only for the desktop installer; `.exe` URL comes from GitHub `releases/latest` via `GET /api/app/latest-release` (`demirrsarppkurtlarr/Descall`). CI publishes `Descall-Setup-*.exe` on version tags. Bump semver with `frontend/electron/release.cjs` (`patch` / `minor` / `major`). See `docs/ELECTRON_RELEASE.md`.
